...
//  Throws on type mismatch
    ...
    void Revise() {
        m_ref.get().m_rev.SetDataRevision(m_ref.get().m_rev.GetDataRevision() + 1);
    }
...
public:
    ZDO(ZDOBase& zdo, ZDOID id) : m_ref(zdo), m_id(id) {}
    ...
    [[maybe_unused]] bool _SetConnection(ZDOConnector::Type type, ZDOID zdoid) {
        auto&& insert = m_ref.get().ZDO_TARGETED_CONNECTORS.insert({ ID(),
    ...
    ZDOID GetConnectionZDOID(ZDOConnector::Type type) const {
        auto&& find = ZDOBase::ZDO_TARGETED_CONNECTORS.find(ID());
        if (find != ZDOBase::ZDO_TARGETED_CONNECTORS.end()) {
