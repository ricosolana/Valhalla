...
struct ankerl::unordered_dense::hash<avledet::util::CSU::Vector2i> {
    ...
    auto operator()(avledet::util::CSU::Vector2i const& value) const noexcept -> uint64_t {
...
struct ankerl::unordered_dense::hash<avledet::util::CSU::Vector3f> {
    ...
    auto operator()(avledet::util::CSU::Vector3f const& value) const /*noexcept*/ -> uint64_t {
        (void)value;
        throw std::runtime_error("fix vector padding");
        //return ankerl::unordered_dense::detail::wyhash::hash(&value, sizeof(value));
    }
};
...
using Vector2f = avledet::util::CSU::Vector2<float>;
using Vector2i = avledet::util::CSU::Vector2<int32_t>;
using Vector2s = avledet::util::CSU::Vector2<int16_t>;
using Vector3f = avledet::util::CSU::Vector3<float>;