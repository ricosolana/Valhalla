...
namespace avledet::util {
    ...
    constexpr Hash get_stable_hash(std::string_view str, std::uint32_t num, std::uint32_t num2, std::uint32_t idx) { // NOLINT(misc-no-recursion)
        //if (str[idx] != '\0') {
        if (idx != str.length()) {
    ...
    constexpr std::pair<avledet::util::Hash, avledet::util::Hash> to_hash_pair(std::string_view key) {
        return {
            get_stable_hash(std::string(key) + "_u"),
            get_stable_hash(std::string(key) + "_i")
        };
    }
