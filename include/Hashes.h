...
namespace avledet::util {
    ...
    constexpr Hash get_stable_hash(std::string_view str, std::uint32_t num, std::uint32_t num2,
                                   std::uint32_t idx)
    {// NOLINT(misc-no-recursion)
        ...
        if (idx != str.length()) {
            ...
            //if (str[idx + 1] != '\0') {
            if (idx + 1 != str.length()) {
                num2 = ((num2 << 5) + num2) ^ (std::uint32_t) str[(std::uint64_t) idx + 1];
                idx += 2;
    ...
    constexpr Hash get_stable_hash(std::string_view str)
    {
    ...
    constexpr std::pair<avledet::util::Hash, avledet::util::Hash> to_hash_pair(std::string_view key)
    {
        return {get_stable_hash(std::string(key) + "_u"), get_stable_hash(std::string(key) + "_i")};
    }
    ...
    namespace hashes {
        ...
        namespace Rpc {
            ...
            static constexpr Hash S2C_RequestSave
                    = __H("SavePlayerProfile");// dedicated server registers this method, why?
            ...
            static constexpr Hash C2S_PlayerData
                    = __H("ServerSyncedPlayerData");// dedicated server registers this method, why?
        ...
        namespace Object {
            ...
            static constexpr Hash ship_construction
                    = -1245442852;// seems to be an unused prefab (literally ship being constructed, spawns a ship after a while)
