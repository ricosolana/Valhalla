...
namespace avledet::util {
    ...
    class Writer : public Stream
    {
      ...
      private:
        void internal_write_bytes(char const *inBuf, std::size_t inBufSize);
    ...
    struct Streamer<T>
    {
        ...
        {
            writer.internal_write_bytes(reinterpret_cast<char const *>(&value), sizeof(value));
        }
    ...
    struct Streamer<T>
    {
        ...
        decltype(auto) operator()(Reader &reader) const
        {
            auto count = static_cast<std::uint32_t>(avledet::util::traits::has_traits_type<T>
                                                            ? reader.read_varint()
                                                            : reader.read<std::int32_t>());

            reader.check_offset(count);

            auto result = T(reader.data() + reader.get_pos(), reader.data() + reader.get_pos() + count);
    ...
    template<class T>
        requires(!std::is_same_v<T, char>
                 && std::is_same_v<std::remove_const_t<std::remove_pointer_t<std::remove_all_extents_t<T>>>,
                                   char>)
    struct Streamer<T>
    {
        void operator()(Writer &writer, T const &value) const
    ...
    template<class T>
        requires(avledet::util::traits::is_iterable<T> && !std::same_as<typename T::value_type, char>)
    struct Streamer<T>
    {
        ...
        decltype(auto) operator()(Reader &reader) const
        {
            ...
            for (decltype(size) i = 0; i < size; i++) {
                value.insert(value.end(), reader.read<typename T::value_type>());
            }
            return value;
        }
    };
    ...
    {//<std::function<void(Writer&)>> {
        ...
        {
            static_assert(!std::is_invocable_v<T, Writer &>,
                          "Reader::read(func) must accept simple readable types (not a 'Writer' as arg)");
