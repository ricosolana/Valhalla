...
class IAvledet
{
  ...
  public:
    ...
    {
        return (double) day * (double) WORLD_TIME_LENGTH + timeOfDay;
    }
    ...
    {
        return GetWorldTime(GetDay(worldTime) + (GetTimeOfDay(worldTime) < timeOfDay ? 0 : 1), timeOfDay);
    }
    ...
    {
        return avledet::util::Ticks(
                (std::int64_t)(this->m_worldTime * (WorldTime) avledet::util::Ticks::period::den));
    }
    ...
    Task &RunTaskAtRepeat(Task::F f, std::chrono::steady_clock::time_point at,
