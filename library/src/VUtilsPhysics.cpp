...
namespace VUtils::Physics {
    ...
    // and 'p2q2' intersect.
    bool LinesIntersect(Vector2f const &p1, Vector2f const &q1, Vector2f const &p2, Vector2f const &q2)
    {
        static auto &&onSegment = [](Vector2f const &p, Vector2f const &q, Vector2f const &r) -> bool {
        ...
        // 1 --> Clockwise
        // 2 --> Counterclockwise
        static auto &&orientation = [](Vector2f const &p, Vector2f const &q, Vector2f const &r) -> int {
    ...
    bool PointInsideRect(Vector3f size, Vector3f pos)
    {
        ...
        return pos.x >= -size.x && pos.x <= size.x && pos.y >= -size.y && pos.y <= size.y && pos.z >= -size.z
               && pos.z <= size.z;
    }
                         ...
                         Quaternion rot2, std::string &desmos)
    {
...

        {
            ...
            if (!(VUtils::Math::Between(a1, b1, b2) || VUtils::Math::Between(a2, b1, b2)
                  || VUtils::Math::Between(b1, a1, a2) || VUtils::Math::Between(b2, a1, a2)))
                return false;
        ...
        ss << "polygon((" << v3_a_br.x << "," << v3_a_br.z << "),(" << v3_a_ur.x << "," << v3_a_ur.z << "),("
