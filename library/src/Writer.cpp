...
namespace avledet::util {
    ...
    Writer::Writer(std::vector<char> buf, std::size_t pos) :
        Stream(std::move(buf), pos)
    {
    }

    void Writer::internal_write_bytes(char const *inBuf, std::size_t inBufSize)
    {
        if (!this->is_valid_offset(inBufSize))
            m_buf.resize(m_pos + inBufSize);

        std::copy(inBuf, inBuf + inBufSize, this->data() + m_pos);
    ...
    void Writer::write_varint(std::int32_t value)
    {
        ...
        for (; num >= 128U; num >>= 7) this->write(static_cast<std::uint8_t>(num | 128U));
