...
struct convert<AssignAlgorithm>
{
    ...
    static bool decode(const Node &node, AssignAlgorithm &rhs)
    {
        ...
        if (auto opt
            = magic_enum::enum_cast<AssignAlgorithm>(node.as<std::string>(), magic_enum::case_insensitive)) {
            rhs = opt.value();
            return true;
        }
...
struct convert<ankerl::unordered_dense::map<K, V, Hash, Eq, Alloc, Bucket>>
{
    static Node encode(const ankerl::unordered_dense::map<K, V, Hash, Eq, Alloc, Bucket> &rhs)
    {
        ...
        for (const auto &element : rhs) node.force_insert(element.first, element.second);
        return node;
    }
...
struct convert<ankerl::unordered_dense::set<K, Hash, Eq, Alloc, Bucket>>
{
    static Node encode(const ankerl::unordered_dense::set<K, Hash, Eq, Alloc, Bucket> &rhs)
    {
        ...
        for (const auto &element : rhs) node.push_back(element);
        return node;
    }
        ...
        if (!reloading || !fileError) {
            ...
            */
            ...
            a(m_settings.serverName, server, "name", "Valhalla server",
              [](const std::string &val) { return val.empty() || val.length() < 3 || val.length() > 64; });
            a(m_settings.serverPassword, server, "password", "",
              [](const std::string &val) { return !val.empty() && (val.length() < 5 || val.length() > 11); });
            ...
            */
            ...
            a(
                    ...
                    [](const std::string &val) { return val.empty() || val.length() < 3; }, reloading);
            a(
                    m_settings.worldSeed, world, "seed", VUtils::Random::GenerateAlphaNum(10),
                    [](const std::string &val) { return val.empty(); }, reloading);
...
IValhalla::~IValhalla()
{
    // Never before tested, might cause a black hole
    Stop();
}
