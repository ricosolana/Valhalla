...
namespace avledet::util {
    ...
    Stream::Stream(std::vector<char> buf) : m_buf(std::move(buf)), m_pos(0) { }
    Stream::Stream(std::vector<char> buf, std::size_t pos) : m_buf(std::move(buf)), m_pos(pos) { check_pos(pos); }
