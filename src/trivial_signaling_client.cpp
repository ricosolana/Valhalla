...
public:
	...
	void Send( const std::string &s )
	{
		...
		{
			printf( "Signaling send queue is backed up.  Discarding oldest signals\n" );
	...
	ISteamNetworkingConnectionSignaling *CreateSignalingForConnection(
		...
		printf( "Creating signaling session for peer '%s'\n", sIdentityPeer.c_str() );
		...
		else
		{
			for (;;)
			{
				...
				if ( r < 0 )
				{
					...
					{
						printf( "Failed to recv from trivial signaling server.  recv() returned %d, errno=%d.  Closing and restarting connection\n", r, e );
						CloseSocket();
					}
					break;
				}
		...
		if ( m_sock != INVALID_SOCKET )
		{
			while ( !m_queueSend.empty() )
			{
				...
				else if ( r != 0 )
				{
					...
					printf( "Failed to send %d bytes to trivial signaling server.  send() returned %d, errno=%d.  Closing and restarting connection.\n",
						l, r, GetSocketError() );
					CloseSocket();
					break;
				}
