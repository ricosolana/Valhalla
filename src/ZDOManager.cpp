...
void IZDOManager::Update() {
	PERIODIC_NOW(2s, {
		for (auto&& peer : NetManager::GetPeers()) {
			ReleaseNearbyZDOS(peer->m_pos, peer->m_uuid);
		}
	});
...
void IZDOManager::ServerSortSendZDOS(std::vector<NetSync*>& objects, const Vector3& refPos, ZDOPeer* peer) {
