...
void IZDOManager::init() {
	...
	RouteManager()->Register(Hashes::Routed::DestroyZDO, 
		[this](Peer*, DataReader reader) {
			...
			reader.for_each([this](ZDOID zdoid) {
				EraseZDO(zdoid);
			});
		}
	);
...
void IZDOManager::on_peer_quit(Peer& peer) {
	for (auto&& itr = m_objectsByID.begin(); itr != m_objectsByID.end(); ) {
		...
		if (!zdo.IsPersistent()
			&& (!zdo.HasOwner() || zdo.IsOwner(peer.GetUserID()) || !NetManager()->get_peer_by_userid(zdo.Owner())))
		{
			itr = _DestroyZDO(itr);
		}
		else
