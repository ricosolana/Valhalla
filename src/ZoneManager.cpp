...
void IZoneManager::PostGeoInit() {
    ...
    if ((VH_SETTINGS.worldCaptureMode != WorldMode::PLAYBACK)
        && (VH_SETTINGS.worldPregenerate || VH_SETTINGS.worldCaptureMode == WorldMode::CAPTURE) 
...
void IZoneManager::GenerateFeature(const Feature& location, HASH_t seed, const Vector3f& pos, const Quaternion& rot) {
    ...
    for (auto&& piece : location.m_pieces) {
        ...
        if (!(VH_SETTINGS.spawningDungeons && piece.m_prefab->AllFlagsPresent(Prefab::Flag::DUNGEON))) {
