...
sol::table IModManager::load_api_table() {
    ...
    table.new_usertype<IValhalla>("IValhalla",
        // server members
    ...
    table.new_usertype<IModManager>("IModManager",
    ...
    table.new_usertype<IRouteManager>("IRouteManager",
...

    {
...

        {
            ...
            resourceUtilsTable["write_file"] = sol::overload(
                ...
                sol::resolve<bool(fs::path const &, std::vector<std::string> const&)>(VUtils::Resource::WriteFile),
                sol::resolve<bool(fs::path const &, std::list<std::string> const&)>(VUtils::Resource::WriteFile)
            );
        }
    }
